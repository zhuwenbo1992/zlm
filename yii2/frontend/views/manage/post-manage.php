...
<div class="row white-bg padding5 border-bottom">
    <div class="col-xs-12 black-color text-right padding5" >
        ...
        <a href="/manage/money-manage?order_id=<?=$val['order_id']?>" class="btn btn-sm btn-default">申请薪资托管</a>
        <?php }?>

        <?php if($val['is_mandate']==1&&$val['order_status']>0){?>
            <a href="/manage/money-manage?order_id=<?=$val['order_id']?>" class="btn btn-sm btn-default">薪资托管(审核中)</a>
        <?php }?>
    <?php if($val['is_mandate']==2){?>
        ...
        <a href="/manage/drop-order?id=<?=$val['order_id']?>" class="btn btn-sm btn-outline btn-primary">取消发单</a>
