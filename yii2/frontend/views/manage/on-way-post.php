...
<?php foreach ($MyPostList as $key=>$val):?>
    ...
    <div class="row white-bg padding5 border-bottom">
        <div class="col-xs-12 black-color text-right padding5" >
            ...
            <?php if($val['order_status']==2&&$val['offer_people']>0){?>

                <a href="/manage/luyong?order_id=<?=$val['take_num'][0]['order_take_id']?>&&is_myself=<?=$val['take_num'][0]['is_myself']?>" style="color:green" class="btn btn-sm btn-outline btn-primary"><?=$val['offer_people']?>人已被录用</a>
            <?php }?>
        </div>
    </div>
        <div class="row" style="height: 5px;"></div>
